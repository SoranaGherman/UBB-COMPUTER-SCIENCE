...
public class Controller {
    ...
    public void allSteps() throws InterruptedException {
        ...
        while (prgList.size() > 0){
            ...
            List<MyIDictionary<String, Value> >allSymTables = prgList.stream().map(PrgState::getTopSymTable).collect(Collectors.toList());
    ...
    public void conservativeGarbageCollector(List<PrgState> programStates) {
        List<Integer> symTableAddresses = Objects.requireNonNull(programStates.stream()
                        .map(p -> getAddressFromSymTable(p.getTopSymTable().values()))
