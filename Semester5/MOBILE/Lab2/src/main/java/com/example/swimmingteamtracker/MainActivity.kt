...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == DELETE_SWIMMER_REQUEST_CODE && resultCode == RESULT_OK) {
            if (data != null) {
                val deletedSwimmerId = data.getIntExtra("deletedSwimmer", -1)
                val deletedSwimmerIndex = swimmersList.indexOfFirst { it.id == deletedSwimmerId }
                if (deletedSwimmerIndex != -1) {
                    swimmersList.removeAt(deletedSwimmerIndex)
                    adapterSwimmersList.notifyItemRemoved(deletedSwimmerIndex)
                }
            }
        }
    }
